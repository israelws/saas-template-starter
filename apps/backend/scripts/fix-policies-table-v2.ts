...
async function fixPoliciesTable() {
  ...
  try {
    ...
    const requiredColumns = ['subjects', 'resources', 'actions', 'conditions', 'isActive', 'version', 'metadata'];
    ...
    // Also check policy_sets table
    console.log('\nChecking policy_sets table structure...');
    const checkPolicySetsQuery = `
      SELECT column_name, data_type, is_nullable, column_default
      FROM information_schema.columns
      WHERE table_name = 'policy_sets'
      AND table_schema = 'public'
      ORDER BY ordinal_position;
    `;
    
    const policySetsColumns = await client.query(checkPolicySetsQuery);
    const policySetsExistingColumns = policySetsColumns.rows.map(row => row.column_name);
    
    if (!policySetsExistingColumns.includes('isactive')) {
      console.log('\nAdding isActive column to policy_sets table...');
      await client.query(`
        ALTER TABLE policy_sets 
        ADD COLUMN IF NOT EXISTS "isActive" BOOLEAN NOT NULL DEFAULT true;
      `);
      console.log('✓ Column isActive added to policy_sets successfully');
    }

    if (!policySetsExistingColumns.includes('metadata')) {
      console.log('\nAdding metadata column to policy_sets table...');
      await client.query(`
        ALTER TABLE policy_sets 
        ADD COLUMN IF NOT EXISTS metadata JSONB;
      `);
      console.log('✓ Column metadata added to policy_sets successfully');
    }

    // Verify the changes
    console.log('\nVerifying changes...');
    const verifyColumns = await client.query(checkQuery);
    console.log('Updated policies columns:');
    ...
    console.log('\n✓ Policies and policy_sets tables fixed successfully!');
