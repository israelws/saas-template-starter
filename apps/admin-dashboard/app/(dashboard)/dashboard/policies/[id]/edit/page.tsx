...
export default function EditPolicyPage() {
  ...
  const fetchPolicy = async () => {
    try {
      ...
      if (policy.conditions && Object.keys(policy.conditions).length > 0) {
        ...
        Object.entries(policy.conditions).forEach(([field, condition]: [string, any]) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await policyAPI.update(params.id as string, policyData)
      toast({
        title: 'Success',
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-500">Update access control policy</p>
      </div>
