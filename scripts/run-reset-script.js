// This script will be executed in the browser console
// First, let's check if we're on the right page and authenticated

if (!window.location.href.includes('/dashboard/organizations')) {
  console.error('Please navigate to the organizations page first!');
} else if (!localStorage.getItem('authToken')) {
  console.error('Not authenticated! Please login first.');
} else {
  console.log('Starting organization reset and population...');
  
  // Load and execute the reset script
  fetch('/reset-and-populate-organizations.js')
    .then(response => response.text())
    .then(script => eval(script))
    .catch(error => {
      console.error('Failed to load script:', error);
      // If fetch fails, we'll execute the script content directly
      
      // Copy the reset script content here
